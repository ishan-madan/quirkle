...
export class CoordinateUtil {
  ...
  static getSpan(
    coordinates: Coordinate[]
  ): { start: Coordinate; end: Coordinate; allCoordinates: Coordinate[] } {
  ...
  static isValid(coordinate: unknown): coordinate is Coordinate {
    ...
    const xPart = parts[0];
    const yPart = parts[1];
    if (xPart === undefined || yPart === undefined) return false;
