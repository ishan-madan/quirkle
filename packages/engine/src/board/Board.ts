...
export class Board {
  private tiles: Map<Coordinate, TileInstance>;
   ...
   */
  getAffectedLines(
    placements: Array<{ coordinate: Coordinate; tile: TileInstance }>,
    ...
    const perpendicular: Coordinate[][] = [];
   ...
   */
  getLineWithTiles(coordinates: Coordinate[]): Array<{ coordinate: Coordinate; tile: TileInstance }> {
    return coordinates
      .map((coord) => ({
        coordinate: coord,
        tile: this.getTile(coord),
      }))
      .filter((item) => item.tile !== undefined) as Array<{
        coordinate: Coordinate;
        tile: TileInstance;
      }>;
  }
