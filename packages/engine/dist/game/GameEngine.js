...
export class GameEngine {
    ...
    constructor(config) {
        ...
        if (config.playerNames.length !== config.playerCount) {
            throw new Error(`playerNames length (${config.playerNames.length}) must match playerCount (${config.playerCount})`);
        }
        // Initialize bag
    ...
    getGameState() {
        return {
            gameId: this.gameId,
            phase: this.phase,
            board: new Map(this.board['tiles']),
     ...
     */
    addTileToDraft(tileId, coordinate) {
    ...
    commitMove() {
        ...
        const validationError = MoveValidator.validatePlacement(placements, player.rack, this.board, isFirstMove);
        if (validationError) {
            return {
                success: false,
