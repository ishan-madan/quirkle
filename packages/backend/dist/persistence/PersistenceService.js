...
export class PersistenceService {
    ...
    async onGameStarted(session, lobby) {
    ...
    async appendSessionEvents(session) {
        ...
        try {
            ...
            for (const entry of pending) {
                const actorDbUserId = entry.actorUserId
                    ...
                    : null;
                await this.games.appendHistory(session.persistedGameId, {
    ...
    async completeGame(session, lobby, state) {
        if (!session.persistedGameId) {
            return;
        }
        ...
        try {
            await client.query('begin');
            const dbUserByExternal = new Map();
            ...
            const externalByPlayerNumber = new Map();
            ...
            const placementByPlayerNumber = new Map();
    ...
    async getUserStatsByExternalUserId(externalUserId) {
